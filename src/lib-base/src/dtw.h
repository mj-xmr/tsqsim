...
*/
static void computeEnvelope(const vector<floattype> & array, uint constraint, vector<floattype> & maxvalues, vector<floattype> & minvalues) {
...
static void piecewiseSumReduction(const vector<floattype> & array, vector<floattype> & out) {
...
/*
static double l1diff(const vector<double> &  v, const vector<double> & w) {
	double ans(0);
	for(uint k = 0; k<v.size();++k)
	  ans+=abs(v[k]-w[k]);
	return ans;
